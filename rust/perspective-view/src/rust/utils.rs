...
pub trait PerspectiveComponent: Clone {
    ...
    fn async_method_to_jsfunction<F, T>(
        &self,
        f: F,
    ) -> (Closure<dyn Fn() -> js_sys::Promise>, js_sys::Function)
    where
        ...
        Self: 'static,
    {
        ...
        let closure = Closure::wrap(box_cb);
        let jsfn = closure.as_ref().unchecked_ref::<js_sys::Function>().clone();
        (closure, jsfn)
    }
...
mod perspective_component_tests {
    ...
    fn test_async_method_to_jsfunction() {
        ...
        let _: js_sys::Function = (Test {}).async_method_to_jsfunction(f).1;
    }
