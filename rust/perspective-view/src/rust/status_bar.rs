...
struct StatusBarState {
    ...
    update_callback: Option<(Closure<dyn Fn() -> js_sys::Promise>, js_sys::Function)>,
}
...
impl StatusBarElement {
    ...
    pub fn set_view(&mut self, view: PerspectiveJsView) -> Result<Promise, JsValue> {
        self.remove_on_update_callback();
        let (cb, jsfn) = self.async_method_to_jsfunction(|this| async move {
            StatusBarState::update_state(&this.state).await?;
            this.render()
        });

        view.on_update(&jsfn);
        self.state.borrow_mut().update_callback = Some((cb, jsfn));
    ...
    fn remove_on_update_callback(&self) {
        if let StatusBarState {
            update_callback: Some((_, ref jsfn)),
            view: Some(ref view),
            ..
        } = *self.state.borrow()
        {
            view.remove_update(&jsfn);
        }
    }
