...
try {
    if (!IS_INSIDE_PUPPETEER && !IS_LOCAL_PUPPETEER) {
        ...
        if (!PACKAGE || minimatch("perspective-view", PACKAGE)) {
            console.log("-- Running Rust tests");
            execute`yarn lerna --scope=@finos/perspective-view exec yarn test`;
        }

        execute`${docker("puppeteer")} node scripts/test_js.js --private-puppeteer ${getarg()}`;
    } else {
        if (!IS_INSIDE_PUPPETEER && (!PACKAGE || minimatch("perspective-view", PACKAGE))) {
            console.log("-- Running Rust tests");
            execute`yarn lerna --scope=@finos/perspective-view exec yarn test`;
        }
